...
//  Calculator-Clone
//
//  Created by SeungMin Lee on 2022/11/15.
//
...
extension CalcControllerViewModel {
    public func didSelectButton(with calcButton: CalculatorButton) {
        ...
        case .plusMinus: self.didSeletPlusMinus()
        ...
        case .equals: self.didSeletEqualsButton()
...
extension CalcControllerViewModel {
    ...
    private func didSeletEqualsButton() {
        
        if let operation = self.operation,
           let firstNumber = self.firstNumber?.toDouble,
           let secondNumber = self.secondNumber?.toDouble {
            
            // Equals is rpressed normally after firstNumber, then an operation, then a secondNumber
...
extension CalcControllerViewModel {
    ...
    private func didSeletPlusMinus() {
